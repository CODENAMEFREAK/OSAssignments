...
FreakMonitor_NON_BUSY_WAIT *nonbusywaitMonitor;
...
void * produce(void * item)
{
	...
	while(1){
		...
		*i=((*i)+randomSleepTime)%10;
		nonbusywaitMonitor->produceToBuffer(*i);
		srand(time((NULL)));
...
void * consume(void * item)
{
	...
	while(1)
	{
		nonbusywaitMonitor->consumeFromBuffer();
		srand(time((NULL)));
		randomSleepTime = (rand()%10);
...
int main()
{
	...
	nonbusywaitMonitor =new FreakMonitor_NON_BUSY_WAIT(sharedBoundedBuffer);
